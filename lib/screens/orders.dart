...
import '../bloc/order_cencel/order_cencel_bloc.dart';
...
class _OrdersState extends State<Orders> {
  ...
  void initState() {
    ...
    context.read<OrderFalseBloc>().add(
      const OrderFalseEvent.orders(active: false),
    );
  ...
  Widget build(BuildContext context) {
    return BlocListener<OrderCencelBloc, OrderCencelState>(
      listener: (context, state) {
        state.whenOrNull(
          success: (message) {
            context.read<OrdersBloc>().add(
              const OrdersEvent.orders(active: true),
            );
            context.read<OrderFalseBloc>().add(
              const OrderFalseEvent.orders(active: false),
            );
            ...
            children: [
              ...
              BlocBuilder<OrdersBloc, OrdersState>(
                builder: (context, state) {
                  return state.when(
                    initial: () =>
                        const Center(child: Text("⏳ Yuklanmoqda...")),
                    loading: () => Center(
                    ...
                    success: (orders) {
                      if (orders.data == null || orders.data!.isEmpty) {
                        return const Center(
                          child: Text("🟡 Aktiv buyurtmalar yo‘q"),
                        );
                      }
